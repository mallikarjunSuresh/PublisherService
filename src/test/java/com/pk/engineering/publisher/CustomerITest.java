...
class CustomerITest {
	...
	void testCustomerPostMethodWhenCalledWithValidRequestShouldReturnSucessResponse() {
		...
		RestAssured.given().header("Authorization", "Bearer " + oauth_token).header("Transaction-Id", "1979c1ce-0fc1-11ec-82a8-0242ac130003").header("Activity-Id", "1ff79f94-0fc1-11ec-82a8-0242ac130003").header("Content-Type","application/json" )
		 .header("Accept","application/json" ).with().body(customerRequest).when()
				.request("POST", "v1/customer").then().assertThat().statusCode(200)
	...
	void testCustomerPostMethodWhenCalledWithInvalidRequestShouldReturnInvalidRequestFailureResponse() {
		...
		RestAssured.given().header("Authorization", "Bearer " + oauth_token).header("Transaction-Id", "1979c1ce-0fc1-11ec-82a8-0242ac130003").header("Activity-Id", "1ff79f94-0fc1-11ec-82a8-0242ac130003").header("Content-Type","application/json" )
		 .header("Accept","application/json" ).with().body(customerRequest).when()
				.request("POST", "v1/customer").then().assertThat().statusCode(400)
				.body("status", Matchers.equalTo("failed")).body("errorType", Matchers.equalTo("Invalid Request Exception"));
	...
	void testCustomerPostMethodWhenCalledWithInvalidHeaderShouldReturnInvalidHeaderFailureResponse() {
		...
		RestAssured.given().header("Authorization", "Bearer " + oauth_token).header("Activity-Id", "1ff79f94-0fc1-11ec-82a8-0242ac130003").header("Content-Type","application/json" )
		 .header("Accept","application/json" ).with().body(customerRequest).when()
				.request("POST", "v1/customer").then().assertThat().statusCode(400)
				.body("status", Matchers.equalTo("failed")).body("errorType", Matchers.equalTo("Invalid Header Exception"));
	...
	void testCustomerPostMethodWhenCalledWithInvalidPathShouldReturnInvalidHandlerFailureResponse() {
		...
		RestAssured.given().header("Authorization", "Bearer " + oauth_token).header("Transaction-Id", "1979c1ce-0fc1-11ec-82a8-0242ac130003").header("Activity-Id", "1ff79f94-0fc1-11ec-82a8-0242ac130003").header("Content-Type","application/json" )
		 .header("Accept","application/json" ).with().body(customerRequest).when()
				.request("POST", "v1/wrongMapping").then().assertThat().statusCode(404)
			 .body("status", Matchers.equalTo("failed")).body("errorType", Matchers.equalTo("Invalid Handler Exception"));
